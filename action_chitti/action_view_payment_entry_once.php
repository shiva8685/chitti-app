...
//alert("res");
 //window.location="notifications.php";
        ...
        xmlhttp.open("GET","action_chitti_payment_save.php?sno="+sno+"&month="+month+"&amount_paid="+amount_paid+"&paid_date="+paid_date+"&payment_mode="+payment_mode+"&status="+status+"&service="+service+"&name="+name+"&mobile="+mobile+"&userid="+userId+"&photo="+photo,true);
        xmlhttp.send();
    }
}

   
  function getPaymentEachMonth(mnth,id,mobile,table,userId){
				...
				if(commission_amount.includes("No")){
					...
					if(this.responseText.includes("must paid full amount")){
						alert(this.responseText);
						 ...
						 $("#amount_paid"+id).prop("disabled", false);
...
if(commission_amount.includes("Due Ledhu")){
	$("#amount_paid"+id).prop("disabled", true);
...
?>
  ...
  function paidAmount(val,id){
	  ...
	  var commisstion = <?php echo $commission_amount; ?>;
	 
	 
	 if(val > parseInt(commisstion)){
		  alert("Your paying extra amount "+(val-commisstion));
 // document.getElementById("saveBtn"+id).style.display = "none";
	  }	  
	  if(val.toString() == commisstion.toString()){
...
<h2>ACTION PAYMENT<br><span class="taglinks" style="padding:10px;color:lightblue;font-size:15px;"><?php echo $_GET['name']; ?> </span>
...
$sqln = "SELECT * FROM action_new_chitti WHERE status = 'published' AND type='action'";
...
<tr id="row<?php echo $row['sno']; ?>">
   ...
   <td>
    ...
    <td>
	<a href="single_user_all_details.php?t=<?php echo $service; ?>&p=<?php echo $row['mobile']; ?>&m=<?php echo $row['months']; ?>&v=<?php echo $row['chitti_value']; ?>&mem=<?php echo $rown["members"]; ?>&n=<?php echo $row['name']; ?>&comsion=<?php echo $rown['chitti_com']; ?>&uid=<?php echo $userID; ?>" > 
	...
	<td>
	 ...
	 <select id="month<?php echo $row['sno']; ?>" onchange="getPaymentEachMonth(this.value,<?php echo $row['sno']; ?>,<?php echo $row['mobile']; ?>,'<?php echo $rown['chitti_number']; ?>','<?php echo $row['user_id']; ?>')" name="month" style="width:100px">
...
?>
   ...
   <td> <input type="number" class="amountPaid" id="amount_paid<?php echo $row['sno']; ?>" onkeyup="paidAmount(this.value,'<?php echo $row['sno']; ?>')" name="amount_paid" style="width:100px" value="">
   
   <span class="showDue"  id="get_commission<?php echo $row['sno']; ?>" style="color:gray"></span>
   </td>
 ...
 <select id="status<?php echo $row['sno']; ?>" name="status" >
	...
	<td><button id="saveBtn<?php echo $row['sno']; ?>" style="display:none;" class="button" style="" onclick="saveData(<?php echo $row['sno']; ?>,'<?php echo $row['user_id']; ?>')">&emsp;Save&emsp;</button></td>

...
error: function(){} 	        
  ...
  if(check == "unchecked"){
	  alert("Please check lifted option");
	 
	  return;
	  
  }
   if(lift_date == ""){
	  alert("Please select lift date");
...
//alert(this.responseText);
 ...
 function checkbox(id){
  ...
  function getSawalAmount(id,cv){
	 //s alert(id+" "+cv);
	  month = document.getElementById("month"+id).value;
	   
		finalAmount = fixedChittiLiftedAmount(cv,month-1);
	  
	  
	  document.getElementById("lifted_amount"+id).value = finalAmount;
	  
	  
  }
  
  function getSawalAmountCheck(val,id,cv){
	 month = document.getElementById("month"+id).value;
	 chittiValue = '<?php echo $_GET['v']; ?>';
	  chittiCom = '<?php echo $_GET['comsion']; ?>';
	 payableAmount = parseInt(chittiCom)+parseInt(1000*chittiValue);
	
	 // payableAmount = (additionalAmount)+(chittiCom);
	 var x;
	$(document).ready(function(){
	 x=$("#checkbox"+id).is(":checked");
	 //alert(x);
	  if(x == true){
	   ...
	   finalAmount = fixedChittiLiftedAmount(cv,month-1);
	 document.getElementById("lifted_amount"+id).value = finalAmount;
	  document.getElementById("amount_pay_each"+id).value = chittiCom;
	   document.getElementById("lifterPay"+id).innerHTML = "Pay Rs:"+ payableAmount;
   document.getElementById("btnSave"+id).style.display = "block";
	
	 }else{
		 
		   document.getElementById("lifted_amount"+id).value = 0;
		  document.getElementById("month"+id).value = 1;
		  document.getElementById("amount_pay_each"+id).value = 0;
		   document.getElementById("lifterPay"+id).innerHTML = "";
  ...
  function fixedChittiLiftedAmount(chittiValue,month){
	...
	talyAmount =  [92500,92500,92500,95000,97500,100000,101000,102000,103000,104000,105000,106000,107000,108000,109000,110000,111000,112000,113000,114000];
...
<h2>FIXED <?php echo strtoupper($_GET["t"]); ?> LIFTING</h2>
...
include("give_user_lifted_amount.php");
...
<tr>
    ...
    <th style="width:110px;display:none;">SAWAL AMOUNT</th>
    <th style="width:110px;display:none;">COMMISSION AMOUNT</th>
...
<tr id="row<?php echo $row['sno']; ?>">
   ...
   <td>
	...
	<p style="color:red;font-size:11px;" id="lifterPay<?php echo $row['sno']; ?>">&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;</p>
	</td>
    <td><?php echo $row['mobile']; ?></td>
	
	<td> 
	<label class="containerCheck">
	<input type="checkbox" onclick="getSawalAmountCheck(this.value,<?php echo $row['sno']; ?>,'<?php echo $_GET['v']; ?>')" id="checkbox<?php echo $row['sno']; ?>"  <?php echo $row['lift_chitti']; ?> >
	...
	<td>
	 ...
	 <select id="month<?php echo $row['sno']; ?>" onchange="getSawalAmount(<?php echo $row['sno']; ?>,'<?php echo $_GET['v']; ?>')"  name="month" style="width:60px">
	<?php
...
while($rowP=mysqli_fetch_array($resP))
{
   ...
   <td style="display:none;"> <input type="number" id="sawal_amount<?php echo $row['sno']; ?>"  name="sawal_amount" style="width:110px" value="<?php echo $row['sawal_amount']; ?>"></td>
    <td style="display:none;"><input type="number" id="com_amount<?php echo $row['sno']; ?>"  name="com_amount" style="width:110px" value="<?php echo $row['commission_amount']; ?>"></td>

	 <td><input class="hi" type="number" id="lifted_amount<?php echo $row['sno']; ?>"  name="lifted_amount" style="width:110px" value="<?php echo $row['lifted_amount']; ?>"></td>
 ...
 <td><input class="hi" type="number" id="amount_pay_each<?php echo $row['sno']; ?>"  name="amount_pay_each" style="width:110px" value="<?php echo $row['amount_pay_by_each']; ?>"></td>
...
?>	
	...
	<!--<button class="button" id="btnSave<?php echo $row['sno']; ?>" style="margin-left: 70px;display:none;" onclick="sawalSave(<?php echo $row['sno']; ?>)">&emsp;Save&emsp;</button>-->
	<?php

...
//alert("res");
 //window.location="notifications.php";
  ...
  function getPaymentEachMonth(mnth,id,mobile,userId,table,cValue){
	  
	   //alert(mnth+" "+id+" "+mobile+" "+userId+" "+table+" "+cValue);
        ...
        xmlhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
				...
				if(commission_amount.includes("No")){
					alert("In this month chitti not lifted yet");
					 document.getElementById("amount_paid"+id).disabled = true;
	 ...
	 $(".mselect").val("SELECT");
	 $(".payAmountDis").html("");
	  $("#get_commission"+id).html("/"+commission_amount);
	  $("#month"+id).val(mnth);
	  document.getElementById("amount_paid"+id).disabled = false;
				});
...
?>
        ...
        xmlhttp.open("GET","get_pay_each_user_month.php?t="+table+"&month="+mnth+"&mobile="+mobile+"&userid="+userId+"&v="+cValue,true);
        xmlhttp.send();
    }
  ...
  function paidAmount(val,id,pAmount,cValue){
	 // alert(val+" "+id+" "+pAmount+" "+cValue);
...
<h2>FIXED PAYMENT<br><span class="taglinks" style="padding:10px;color:lightblue;font-size:15px;"><?php echo $_GET['name']; ?> </span>

<span class="taglinks" style="padding:10px;color:lightblue;font-size:15px;"><?php echo $_GET['user']; ?> </span>
...
<span class="taglinks" style="padding:10px;color:white">CHITTI LISTS </span>
</a>
...
$user = $_GET["user"];
$sqln = "SELECT * FROM action_new_chitti WHERE status = 'published' AND type='fixed' ";
...
$service = $rown['chitti_number'];
$sqld = "SELECT * FROM ".$service." WHERE mobile = '$user' ";
...
<h4 style="color:blue"><b><?php echo $row['chitti_no']; ?>&emsp;<?php echo $row['months']; ?> Months&emsp;<?php echo $row['chitti_value']; ?> Lacks</b></h4>
<table>
<tr>
    <th>SNO</th>
    <th>NAME</th>
...
<tr id="row<?php echo $row['sno']; ?>">
   ...
   <td>
    ...
    <td>
	<a href="single_user_all_details.php?t=<?php echo $service; ?>&p=<?php echo $row['mobile']; ?>&m=<?php echo $row['months']; ?>&v=<?php echo $row['chitti_value']; ?>&mem=<?php echo $rown["members"]; ?>&n=<?php echo $row['name']; ?>&comsion=<?php echo $rown['chitti_com']; ?>&uid=<?php echo $row['user_id']; ?>" > 
	...
	<td>
	 ...
	 <select id="month<?php echo $row['sno']; ?>" onchange="getPaymentEachMonth(this.value,<?php echo $row['sno']; ?>,<?php echo $row['mobile']; ?>,'<?php echo $row['user_id']; ?>','<?php echo $rown['chitti_number']; ?>',<?php echo $rown['chitti_value_lack']; ?>)" name="month" style="width:100px">
	 <option value="SELECT">SELECT</option>
	 <?php
	 
		 $sqlS = "SELECT * FROM ".$service."_sawal WHERE month != 0 ORDER BY month asc";
...
while($rowS=mysqli_fetch_array($resS))
{
	...
	$pm = $rowS['month'];
	 ...
	 $sqlP = "SELECT * FROM ".$service."_payments WHERE month = '$pm' AND status='DUE' AND user_id='$userID'";
...
?>
	  <option value="<?php echo $fMonth; ?>"><?php echo $fMonth; ?> </option>
	<?php  
}else{
...
?>
 <option value="SELECT">NO SAWAL </option>
<?php
}
?>
   ...
   <?php
$userId = $row['user_id'];
$sqlc = "SELECT * FROM ".$service."_sawal WHERE user_id = '$userId' AND lift_chitti = 'checked' ";
...
?>
   ...
   <td> <input class="hi" type="number" id="amount_paid<?php echo $row['sno']; ?>" onkeyup="paidAmount(this.value,'<?php echo $row['sno']; ?>',<?php echo $pAmount; ?>,<?php echo $row['chitti_value']; ?>)" name="amount_paid" style="width:100px" value="">
   
   <span class="payAmountDis"  id="get_commission<?php echo $row['sno']; ?>" style="color:gray"></span>
   </td>
 ...
 <select id="status<?php echo $row['sno']; ?>" name="status" >
	...
	<td><button id="saveBtn<?php echo $row['sno']; ?>" class="button" style="" onclick="saveData(<?php echo $row['sno']; ?>,'<?php echo $row['user_id']; ?>')">&emsp;Save&emsp;</button></td>
